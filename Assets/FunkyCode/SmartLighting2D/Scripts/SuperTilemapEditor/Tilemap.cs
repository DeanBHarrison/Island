...
using CreativeSpore.SuperTilemapEditor;
    ...
    namespace SuperTilemapEditorSupport {
        ...
        public class TilemapCollider : LightingTilemapCollider.Base {
            public List<Transform> colliderTransform = new List<Transform>();
            public List<Polygon2D> localColliders = new List<Polygon2D>();
            public List<Polygon2D> worldColliders = new List<Polygon2D>();
            ...
            public void InitializeGrid() {
                ...
                for (int x = tilemap.MinGridX; x <= tilemap.MaxGridX; x++) {
                    for (int y = tilemap.MinGridY; y <= tilemap.MaxGridY; y++) {
                        ...
                        if (dynamic) {
                            List<Polygon2D> polygons = TileColliderDataToPolygons(tileSTE.collData, tile.scale);
            ...
            List<Polygon2D> TileColliderDataToPolygons(TileColliderData tileColliderData, Vector2 scale) {
                List<Polygon2D> polygons = new List<Polygon2D>();

                if (tileColliderData.vertices.Length > 0) {
                        
                    Polygon2D polygon = new Polygon2D();
                    foreach(Vector2 v in tileColliderData.vertices) {
                        polygon.AddPoint(v - new Vector2(0.5f, 0.5f));
                    }
                ...
                foreach(Transform transform in gameObject.transform) {
                    ...
                    if (edgeColliders.Length > 0) {
                         foreach(EdgeCollider2D collider in edgeColliders) {
                            Polygon2D poly = Polygon2DListCollider2D.CreateFromEdgeCollider(collider);
                    ...
                    if (polygonColliders.Length > 0) {
                        foreach(PolygonCollider2D collider in polygonColliders) {
                            Polygon2D poly = Polygon2DListCollider2D.CreateFromPolygonColliderToLocalSpace(collider)[0];
            ...
            public List<Polygon2D> GetWorldColliders() {
                worldColliders.Clear();

                for(int i = 0; i < localColliders.Count; i++) {
                    Polygon2D polygon = localColliders[i];
